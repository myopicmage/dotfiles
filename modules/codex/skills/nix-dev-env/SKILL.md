...
agent worktrees’ `node_modules` directories — into the nix store on every evaluation.
...
```bash
flake_shell_dir=$(mktemp -d)
cp flake.nix flake.lock "$flake_shell_dir/"
nix develop "path:$flake_shell_dir" --command <cmd>
```
